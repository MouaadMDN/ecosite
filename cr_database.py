...
class Categories(db.Model):
    ...
    articles = db.relationship('Articles', backref='categories', lazy=True)
...
class Articles(db.Model):
    ...
    categories_id = db.Column(db.Integer, db.ForeignKey('categories.id_categorie'), nullable=False)
...
def addarticle(name, editor1, prix, stock, image_name, status, categories_id):
    categorie = Categories.query.filter_by(name=categories_id).first()
    if categorie is None :
        categorie = Categories(name=categories_id)
        db.session.add(categorie)
        db.session.commit()

    new_article = Articles(name=name, stock=stock, editor1=editor1, prix=prix, image=image_name, status=status,categories_id=categories_id)
